...
**국토교통부 건축HUB 3개 API를 8개 도구로.** 건축물대장·건축인허가·주택인허가 + 법정동코드 조회 + **용도지역 포함 종합카드** + **층별 구성** + **동 단위 통계** + **노후건축물 분석**을 AI 어시스턴트에서 자연어로 바로.
...
이 프로젝트는 그 건축 데이터 시스템을 **8개 도구**로 감싸서, AI 어시스턴트나 스크립트에서 바로 호출할 수 있게 만든다. 코드 한 줄 모르는 실무자도 "자양동 노후건물 뽑아줘"라고 말하면 끝.
...
| **건축사·시공** | `building_profile` · `building_floors` · `building_permit` | 용도지역·규모 규제 확인, 층별 구성 파악, 인허가 이력 |
| **디벨로퍼** | `building_profile` · `district_stats` · `old_buildings` | 용도지역·노후 분포로 사업성 1차 스크리닝 |
| **중개** | `building_profile` · `building_floors` | 매물 종합카드 + 층별 임대구성 한 장 |
| **감정평가** | `building_profile` · `building_ledger` | 용도지역·면적·승인일·주차, 주택가격 원천 조회 |
...
**🏗️ 건축사·시공 — 대지 규제·건물 스펙·층별 구성**
```
자양동 2-2번지 용도지역이랑 건폐율·용적률 현황 한눈에 보여줘   → building_profile
자양동 2-2번지 층별로 무슨 용도가 몇 ㎡씩 있는지 보여줘       → building_floors
자양동 최근 건축인허가(신축·증축·대수선) 현황 보여줘          → building_permit(기본개요)
```

**💰 디벨로퍼 — 지역 스크리닝·사업성 1차 판단**
```
자양동 2-2번지 용도지역 뭐고 용적률 얼마나 받았어?           → building_profile
