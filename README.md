...
자양동은 몇 년대에 지어진 건물이 제일 많아? 연대별로 보여줘   → district_stats
```

**🏗️ 건축사·시공 — 건물 스펙·인허가 이력**
```
자양동 2-2번지 건물 구조·규모·건폐율·용적률 한눈에 보여줘   → building_profile
자양동 최근 건축인허가 현황 보여줘                       → building_permit(기본개요)
자양동 2-2번지 건물 층별 개요 알려줘                     → building_ledger(층별개요)
```

**💰 디벨로퍼 — 지역 스크리닝·사업성 1차 판단**
```
자양동 용도별 건물 구성이랑 노후 분포로 사업성 훑어줘        → district_stats
자양동에서 30년 넘은 단독주택 노후 필지 리스트업해줘         → old_buildings
자양동 단독주택이 전체에서 몇 %고 평균 몇 층이야?           → district_stats
```

**🤝 중개 — 매물 스펙 카드 한 장**
```
자양동 2-2번지 건물 스펙 카드로 정리해줘, 손님한테 보여줄거야   → building_profile
이 주소 건물 사용승인일이랑 연면적, 주차대수 알려줘           → building_profile
```

**📊 감정평가 — 공시가격·물건 명세**
```
자양동 24-28번지 공동주택 공시가격 조회해줘                → building_ledger(주택가격)
