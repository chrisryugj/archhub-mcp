...
> 용도지역·정화조(형식·용량 — 용도변경 시 오수발생량 검토의 출발점) 정보가 없는 필지는 해당 줄을 조용히 생략한다.

### 1-1. `parcel_history` — 필지 연혁 타임라인 ★NEW

건축인허가 기본개요 + 철거멸실관리대장 + 가설건축물을 한 번에 받아 **"이 땅에 무슨 일이 있었나"를 오래된 순 타임라인**으로. 허가→착공→사용승인 진행, 철거(석면 ⚠), 가설건축물 존치까지. 민원 대응·이력 확인(공무원), 매입 실사(디벨로퍼·중개·감정평가)의 출발점.

```
"자양동 2-2번지 이 필지에 무슨 일 있었는지 연혁 정리해줘"
→ parcel_history(11215, 10500, bun=2, ji=2)
```

```
[필지 연혁] 2-2 · 인허가 + 철거멸실 + 가설건축물 (오래된 순)

■ 2018-04-18 허가 — 신축 · 업무시설 · 연면적 28,515.35㎡ · 366호 — 건대입구역자이엘라
  착공 2019-09-26 → 사용승인 2022-10-07
■ 2019-03-01 철거멸실 — 철거 · 문화및집회시설 · 철골콘크리트구조 · 연면적 4,697.48㎡ (2019-03-01~2019-04-30)
■ 2019-07-10 허가 — 가설건축물 · 컨테이너조 · 연면적 171.00㎡ · 2개 동 · 존치만료 2022-08-20(경과)
■ 2023-11-30 허가 — 대수선 · 업무시설
  사용승인 2023-12-01
■ 2024-10-18 허가 — 용도변경 · 업무시설 — 건대입구역자이엘라
...
※ 건축HUB 인허가·철거·가설건축물 대장 기준 — 등재 지연·누락 가능. 공부와 대조해 확정하세요.
```

> 옛 건물(건국웨딩타운) 철거 → 신축(자이엘라) → 대수선 → 용도변경까지 한 필지의 생애가 한 화면에 — 전부 자양동 2-2 실측 데이터다.

### 1-2. `temp_buildings` — 가설건축물 존치 스캔 ★NEW

가설건축물 대장을 받아 `가설건축물존치만료일` 기준 **만료 경과 / 만료임박(180일) / 유효 / 미상**을 집계하고 우선 점검 후보를 만료 경과 오래된 순으로 제시. **존치기간 연장 신고 관리는 건축행정의 반복 업무** — 동 단위 일괄 스캔으로 끝낸다. (건축직 공무원·시공 현장 가설사무소 관리)

```
"자양동 가설건축물 중에 존치기간 만료된 것들 점검 목록 뽑아줘"
→ temp_buildings(11215, 10500)
```
...
이 프로젝트는 그 건축 데이터 시스템을 **12개 도구**로 감싸서, AI 어시스턴트나 스크립트에서 바로 호출할 수 있게 만든다. 코드 한 줄 모르는 실무자도 "자양동 노후건물 뽑아줘"라고 말하면 끝.
...
"자양동 1-1번지 표제부 원본 보여줘"      → building_data(ledger, 표제부)  # 원천 데이터
```
...
| 도구 | 설명 |
|------|------|
| `find_region` | 주소 키워드("광진구 자양동 680-63") → `sigungu_code`/`bdong_code` + 번지 자동 분리(`bun`/`ji`) |
| `building_profile` | **용도지역·정화조 포함 종합카드** — 용도지역·주용도·구조·규모·건폐율/용적률·세대·주차·사용승인·내진·에너지·정화조 (건축·시공·중개·평가) |
| `parcel_history` | **필지 연혁 타임라인** ★ — 인허가(허가→착공→승인) + 철거(석면) + 가설건축물을 오래된 순으로 (민원 대응·매입 실사) |
| `temp_buildings` | **가설건축물 존치 스캔** ★ — 존치만료 경과/임박(180일)/유효/미상 분류 + 우선 점검 후보 (건축행정 공무원) |
...
| `seismic_check` | **내진 취약 추정 스캔** — 내진 의무화 연혁(1988→2017.12) 기준 미적용 추정 분류 + 취약구조 우선 (안전점검) |
| `demolitions` | **철거멸실 현황** — 최근 철거순 + 석면 함유 부위 ⚠ (디벨로퍼·철거업체·공무원) |
| `permits_pipeline` | **인허가 파이프라인** — 사용승인 전 진행중 건, 착공/미착공 단계 (디벨로퍼·공무원) |
| `building_data` | 원천 데이터 직접 조회(고급) — `kind=ledger`(건축물대장 10종) / `permit`(건축인허가 17종) / `housing`(주택인허가 16종) |

> v0.4.0에서 raw 3종(`building_ledger`/`building_permit`/`housing_permit`)을 `building_data` 하나로 통합했다 — 파라미터가 동일하고 대장 종류만 달라, 도구 수를 줄여 LLM의 도구 선택 혼란과 스키마 토큰을 같이 줄였다. 원본 표 출력은 빈 컬럼·코드/코드명 중복·조회인자 에코를 자동 생략(생략 수 명시)해 토큰을 아낀다.
...
| 페르소나 | 주력 도구 | 시나리오 |
|---|---|---|
| **건축직 공무원·공공기관** | `parcel_history` · `temp_buildings` · `district_stats` · `old_buildings` · `seismic_check` · `demolitions` · `permits_pipeline` | 민원 필지 연혁 확인, 가설건축물 존치 관리, 동 단위 노후도·내진 집계, 정비·안전점검 대상 선별 |
| **건축사·시공** | `building_profile` · `building_floors` · `district_stats` · `parcel_history` | 용도지역·규모 규제·정화조, 층별 구성, 동네 규모 벤치마크, 필지 인허가 이력 |
| **디벨로퍼** | `parcel_history` · `district_stats` · `old_buildings` · `demolitions` · `permits_pipeline` | 필지 실사, 노후 분포·철거·진행중 인허가로 사업성·공급 스크리닝 |
| **중개** | `building_profile` · `building_floors` · `price_history` · `parcel_history` | 매물 종합카드 + 층별 임대구성 + 공시가 추이 + 매물 이력 |
| **감정평가** | `building_profile` · `price_history` · `parcel_history` · `building_data` | 용도지역·면적·승인일·주차 + 공시가격 시계열 + 원천 대장 |

### 바로 쓰는 프롬프트 (복사 → 지역/번지만 바꾸기)

**처음이라면 이것만 기억하면 된다:**

1. **그냥 말하면 된다.** 코드도, 메뉴도, API 키도 없다. 커넥터만 등록돼 있으면 채팅창에 "자양동 2-2번지 건물 정보 보여줘"라고 치는 게 전부다.
2. **주소는 "동 이름 + 번지"면 충분하다.** "광진구 자양동 680-63"처럼 쓰면 AI가 알아서 지역코드를 찾고 번지를 분리해 조회한다. 번지를 몰라도 동 이름만으로 동 전체 통계를 낼 수 있다.
3. **목적을 말하면 도구는 AI가 고른다.** "정비사업 검토용이야", "손님한테 보여줄 거야"처럼 용도를 덧붙이면 결과 정리가 더 좋아진다.
4. **없는 데이터는 없다고 답한다.** 모든 숫자는 국토교통부 건축HUB 공식 API 실측값이고 출처가 붙는다. 조회 결과가 없으면 지어내지 않고 `[NOT_FOUND]`로 알려준다.

아래는 전부 자양동(광진구) 실제 데이터로 동작 확인된 예시다. **"자양동"을 우리 동네로, 번지를 실제 번지로 바꾸면 그대로 작동**한다. (`→`는 내부적으로 호출되는 도구)

**🏛️ 건축직 공무원 — 민원·점검·현황 실전 쿼리**
```
자양동 2-2번지 이 필지에 무슨 일 있었는지 연혁 정리해줘 (민원 대응용)   → parcel_history
자양동 가설건축물 중 존치기간 만료된 것들 점검 목록 뽑아줘             → temp_buildings
자양동 내진설계 안 됐을 것으로 추정되는 3층 이상 건물 뽑아줘           → seismic_check
...
자양동 2-2번지 정화조 형식이랑 용량 알려줘 (용도변경 검토용)           → building_profile
```
...
자양동 5-13번지 총괄표제부 원본 보여줘                     → building_data(ledger, 총괄표제부)
```
